...
/**
 * Enhanced MessageContext that provides access to EAF security context information extracted from
 * message headers.
 *
 * This context automatically extracts tenant ID, user ID, and correlation ID from message metadata
 * and makes them available to event handlers.
 */
interface EafMessageContext : MessageContext {
    /**
     * The user ID extracted from message headers, if available. This represents the user who
     * initiated the action that led to this event.
     */
    val userId: String?

    /**
     * The correlation ID for tracing requests across services. This can be used to correlate events
...
/**
 * Default implementation of EafMessageContext that extracts security context from NATS message
 * headers.
 */
class DefaultEafMessageContext(
    ...
    override val userId: String? by lazy { getHeader(USER_ID_HEADER) }

    override val correlationId: String? by lazy { getHeader(CORRELATION_ID_HEADER) }
...
object EafMessageContextFactory {
    /**
     * Creates an EafMessageContext from a NATS message. Automatically extracts tenant ID and other
     * context information.
     */
    ...
    /**
     * Extracts tenant ID from the message subject or headers. The tenant ID is expected to be the
     * first part of the subject.
     */
    private fun extractTenantId(message: Message): String {
        // First try to get from headers
        message.headers?.getFirst("eaf.tenant.id")?.let {
            return it
        }
        ...
        throw IllegalArgumentException(
            "Cannot extract tenant ID from message subject: ${message.subject}",
        )
    }
}
