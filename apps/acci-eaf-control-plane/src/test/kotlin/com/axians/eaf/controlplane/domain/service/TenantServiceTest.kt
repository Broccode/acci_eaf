...
class TenantServiceTest {
    ...
    fun `should get tenant details successfully when tenant exists`() = runTest {
        ...
        coEvery { tenantRepository.findById(eq(tenant.id)) } returns tenant
    ...
    fun `should return not found when getting details for non-existent tenant`() = runTest {
        ...
        coEvery { tenantRepository.findById(eq(TenantId.fromString(tenantId))) } returns null
    ...
    fun `should update tenant successfully when new name is unique`() = runTest {
        ...
        coVerify { tenantRepository.findById(eq(tenant.id)) }
    ...
    fun `should check if tenant is operational correctly`() = runTest {
        ...
        coEvery { tenantRepository.findById(eq(activeTenant.id)) } returns activeTenant
        coEvery { tenantRepository.findById(eq(suspendedTenant.id)) } returns suspendedTenant
        coEvery { tenantRepository.findById(eq(TenantId.fromString("non-existent"))) } returns null
