...
class SystemInitializationService(
    ...
    /**
     * Initialize the system with default tenant and superadmin user if required. This operation is
     * idempotent - it will not create duplicates if the system is already initialized.
     *
    ...
    fun initializeDefaultTenantIfRequired(): InitializationResult {
        ...
        if (saveTenantPort.existsByTenantName(properties.defaultTenantName)) {
            val message =
                "System already initialized - default tenant '${properties.defaultTenantName}' exists"
            logger.info(message)
        ...
        if (saveTenantPort.existsByEmail(properties.defaultSuperAdminEmail)) {
            val message =
                "System already initialized - superadmin email '${properties.defaultSuperAdminEmail}' exists"
            logger.info(message)
            return InitializationResult(wasInitialized = false, message = message)
        }
        ...
        try {
            ...
            logger.info(
                "Created default tenant: ${result.savedTenant.name} (ID: ${result.savedTenant.tenantId})",
            )
            logger.info(
            ...
            logger.info(
                "INVITATION PLACEHOLDER: SuperAdmin account created for ${result.savedTenantAdmin.email}",
            )
            logger.info(
                "INVITATION PLACEHOLDER: Please set up password and access tenant '${result.savedTenant.name}'",
            )
