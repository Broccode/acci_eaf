 ...
 *
 * This configuration class sets up all the necessary beans for the ticket management pilot service,
 * integrating EAF SDK components with the application layer.
 */
@Configuration
@EnableMethodSecurity(prePostEnabled = true)
@EnableConfigurationProperties
@EnableJpaRepositories(
    basePackages = ["com.axians.eaf.ticketmanagement.infrastructure.adapter.outbound"],
)
@EntityScan(
    basePackages = ["com.axians.eaf.ticketmanagement.infrastructure.adapter.outbound.entity"],
)
...
class TicketManagementConfiguration {
    ...
    /**
     * Creates the NatsEventPublisher bean using auto-configured connection. Note: The NATS
     * connection and properties are auto-configured by the EAF eventing SDK.
     */
    @Bean
     ...
     *
     * Note: For this pilot, we're using the read model-based query handler for better performance
     * and proper ticket listing functionality.
     */
    @Bean
    ...
    /**
     * Creates the read model-based query handler for optimized queries. This demonstrates the
     * query-side benefits of CQRS with read models.
     */
    @Bean
